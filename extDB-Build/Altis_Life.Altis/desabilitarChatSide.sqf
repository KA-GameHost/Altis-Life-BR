			...
			if (ctrlText ((findDisplay 63) displayCtrl 101) == "Side Channel") then
			{
				...
				if (disconnect_me == 0) then {
					if (isNil "warn_one") then {
						warn_one = true;
						systemChat ("Por favor, nao use voz no chat Side. Esse eh seu primeiro aviso.");
						[] spawn DS_slap_them;
						//["beat04"] spawn DS_really_loud_sounds;
						["PROIBIDO VOZ NO SIDE, USER APENAS TEXTO"] spawn DS_double_cut;
					};
				};
				if (disconnect_me == 1) then {
					if (isNil "warn_two") then {
						warn_two = true;
						systemChat ("Por favor leia as regras apertando M, nao insista em usar voz no chat Side. Esse eh seu segundo aviso e voce vai ser punido se continuar!");
						[] spawn DS_slap_them;
						//["beat04"] spawn DS_really_loud_sounds;
						["PROIBIDO VOZ NO SIDE, USE APENAS TEXTO"] spawn DS_double_cut;
					};
				};
				if (disconnect_me == 2) then {
					if (isNil "warn_three") then {
						warn_three = true;
						systemChat ("POR FAVOR NAO USE VOZ NO CHAT SIDE! Esse eh o seu ULTIMO aviso!");
						systemChat ("Ou voce vai ser congelado e entao desconectado!");
						[] spawn DS_slap_them;
						//["beat04"] spawn DS_really_loud_sounds;
						["PROIBIDO VOZ NO CHAT SIDE! ULTIMO AVISO!"] spawn DS_double_cut;
					};
				};
				if (disconnect_me >= 3) then {
					if (isNil "warn_last") then {
						...
						[] spawn DS_slap_them;
						//["beat04"] spawn DS_really_loud_sounds;
						["Voce foi avisado..."] spawn DS_double_cut;
						1 fademusic 10;
						1 fadesound 10;
						disableUserInput true;
						startLoadingScreen ["Voce esta sendo expulso do servidor"];
